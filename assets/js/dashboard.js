...
copyJsonEntryBtn.addEventListener("click", async () => {
  ...
  if (!jsonText) {
    setFormMessage("Create or edit an invoice first to generate JSON.", "error");
    return;
  }
...
loadGithubBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (result.usedPublicFallback) {
      setSyncMessage(
        `Loaded ${invoices.length} invoice${invoices.length === 1 ? "" : "s"} from public GitHub read. Token is invalid for authenticated actions.`,
        "error"
      );
    } else {
      setSyncMessage(
...
function initializeGithubSyncSection() {
  ...
  setSyncMessage(
    "Manual mode supported: create invoice, copy JSON entry, paste into data/invoices.json, then commit/push.",
    "info"
  );
}
...
function getActiveGithubToken() {
  return String(ghTokenInput.value || "").trim();
}

function markUnsyncedChanges(prefix) {
  setSyncMessage(
    `${prefix} Copy the generated JSON entry and paste it into data/invoices.json, then commit/push.`,
    "info"
  );
}
