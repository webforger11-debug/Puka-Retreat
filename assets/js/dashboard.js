...
const tokenActionsWrap = document.querySelector(".sync-token-actions");
const tokenFieldWrap = ghTokenInput.closest(".field");
...
function initializeGithubSyncSection() {
  ...
  ghTokenInput.value = bundledGithubToken || savedToken;

  if (bundledGithubToken) {
    rememberTokenCheckbox.checked = false;
    if (tokenActionsWrap) {
      tokenActionsWrap.classList.add("hidden");
    }
    if (tokenFieldWrap) {
      tokenFieldWrap.classList.add("hidden");
    }
    setSyncMessage(
      "Bundled token mode active. Use \"Load From GitHub\" then \"Save To GitHub\" after edits.",
      "info"
    );
    return;
  }
...
function getActiveGithubToken() {
  return bundledGithubToken || String(ghTokenInput.value || "").trim();
}
