...
function buildPermissionHelpMessage(payload, fallback) {
  const apiMessage = getErrorMessage(payload, fallback);
  const normalized = normalizeErrorMessage(apiMessage);

  if (normalized.includes("resource not accessible by personal access token")) {
    return "Token cannot access this repo for writes. In fine-grained token settings choose owner webforger11-debug, select repo Puka-Retreat, and set Contents: Read and write.";
  }

  if (
    normalized.includes("branch protection") ||
    normalized.includes("repository rule") ||
    normalized.includes("protected branch")
  ) {
    return "GitHub branch rules are blocking direct writes to this branch. Either relax rules on main or save to a branch that allows direct updates.";
  }

  if (normalized.includes("saml") || normalized.includes("single sign-on")) {
    return "Token is not authorized for SSO on this account/org. Authorize the token for SSO in GitHub token settings.";
  }
...
export async function loadInvoicesFromGithub(config, token) {
  ...
  if (!response.ok && token && response.status === 401) {
    ...
    usedPublicFallback = response.ok;
  }
    ...
    if (response.status === 403) {
      throw new Error(
        buildPermissionHelpMessage(
          payload,
          "Token cannot read this file. Check repo access and token permissions."
        )
      );
    }
    throw new Error(
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      throw new Error(
        buildPermissionHelpMessage(
          payload,
          "Token lacks write permission. Set repository permission Contents: Read and write."
        )
      );
    }
    throw new Error(
